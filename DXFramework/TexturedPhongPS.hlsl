...
float4 main(float3 cameraPos : POSITION, float3 n : NORMAL, float2 texCrd : TEXCOORD) : SV_TARGET
{
	...
	// final color
    return float4(saturate(diffuse + ambient + specular), 1.0f) * tex.Sample(smplr, texCrd);
}